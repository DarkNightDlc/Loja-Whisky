...
export function Header(){
  ...
  function handleFilterProdutos(e){
    setProdutosExibidos(produtos.filter(item=>  item.name.toUpperCase >= e.target.value.toUpperCase))
    ...
    return (
        ...
        <label className='bg-white h-10 max-w-[600px] w-full flex items-center rounded-md'>
          <input 
            type="text" 
            className="border-none h-full w-full min-w-[200px] bg-transparent"
            onChange={handleFilterProdutos}
          />
